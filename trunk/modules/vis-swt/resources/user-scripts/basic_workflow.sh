...
getTestFileName()
{
	testfile_path="swt-aut/$app_folder_name/resources/test_input_file"
	if [ -d $testfile_path ]
	then
		current_path=`pwd`
...
moveFileAndBuild()
{
	defaultIFS=$IFS
	IFS=.
	source_path="./swt-aut/$app_folder_name/src"
	class_path="./swt-aut/$app_folder_name/bin"
	for token in $1
	do
		echo $token
		source_path+="/$token"
	...
	rm $class_path
	ant -f ./swt-aut/$app_folder_name/build.xml build
...
runPhase1()
{
	...
	classpath=./swt-aut/$app_folder_name/bin:$guitar_classpath
	result_dir=./swt-aut/$app_folder_name/results
	ripper_launcher=edu.umd.cs.guitar.ripper.SitarRipperMain
	configuration="./swt-aut/$app_folder_name/guitar-config/configuration.xml"
...
do
	...
	result_dir=./swt-aut/$app_folder_name/results
	faulted_source_dir=./swt-aut/$app_folder_name/faulted_src
	...
	ant -f ./swt-aut/$app_folder_name/build.xml -Dargs="-c $app_name -g ./results/GUITAR-Default.GUI" SitarRipper
