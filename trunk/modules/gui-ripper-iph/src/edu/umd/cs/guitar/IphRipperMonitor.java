...
import edu.umd.cs.guitar.event.IphPickerWheelEvent;
import edu.umd.cs.guitar.event.IphTextFieldEvent;
...
public class IphRipperMonitor extends GRipperMonitor {
	...
	public void setUp() {
		...
		List<Class<? extends GEvent>> DEFAULT_SUPPORTED_EVENTS = Arrays.asList(IphTouchEvent.class,
				IphPickerWheelEvent.class, IphTextFieldEvent.class);
		// for (Class<? extends JFCEventHandler> event : JFCConstants.DEFAULT_SUPPORTED_EVENTS) {
		for (Class<? extends GEvent> event : DEFAULT_SUPPORTED_EVENTS) {
			try {
	...
	public void expandGUI(GObject component) {
		...
		action = new IphPickerWheelEvent();
		if (action.isSupportedBy(component))
			action.perform(component, null);

		action = new IphTextFieldEvent();
		if (action.isSupportedBy(component))
			action.perform(component, null);
   ...
   /**
    * This method is a place holder for ADR.
    */
   @Override
   public void
