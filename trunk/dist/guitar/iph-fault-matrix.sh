...
#   	test_name=${test_name%.*}
    ...
    # Removed -c
   	...
   	cmd1="$SCRIPT_DIR/iph-replayer.sh -cp $aut_classpath -g $gui_file -e $efg_file -t ./Demo/testcases/ToBeReplaced.tst -i $initial_wait -d $replayer_delay -so $replayer_so -l $logs_dir/ToBeReplaced.log -gs $states_dir/ToBeReplaced.sta -cf $SCRIPT_DIR/configuration.xml &> replayer.out &"
   	# adding application arguments if needed 
   	if [ ! -z $args ] 
   	then 
   		cmd="$cmd -a \"$args\" " 
	...
	perl $SCRIPT_DIR/faultSeedingV3.pl $1 $cmd1 $cmd $xcode_replay_time 
