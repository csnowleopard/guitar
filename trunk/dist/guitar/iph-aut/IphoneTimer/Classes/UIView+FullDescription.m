...
- (NSMutableDictionary *)fullDescriptionDict
{
	...
	if ([NSStringFromClass([self class]) isEqual:@"UIPickerView"]) {
		NSLog(@"Picker view!");
		NSString *eventAttributes = [NSString stringWithFormat:@"%@%@", property_list,
									 @"<Property><Name>INVOKE</Name><Value>PICKER_WHEEL</Value></Property>"];
		property_list = eventAttributes;
		//[description setValue:@"INVOKE_TOUCH_TABLE" forKey:@"tableView:willSelectRowAtIndexPath:"];
	}
	
	if ([NSStringFromClass([self class]) isEqual:@"UITextField"]) {
		NSLog(@"UITextField view!");
		NSString *eventAttributes = [NSString stringWithFormat:@"%@%@", property_list,
									 @"<Property><Name>INVOKE</Name><Value>TEXT_FIELD</Value></Property>"];
		property_list = eventAttributes;
		//[description setValue:@"INVOKE_TOUCH_TABLE" forKey:@"tableView:willSelectRowAtIndexPath:"];
	}
...
- (UIView *)findViewWithClass:(NSString *)class andAddress:(NSNumber *)address {
	NSLog(@"[Current view: %@ (addr: %@)] Looking for view: %@, with address: %@.",
		  NSStringFromClass([self class]),
		  [NSNumber numberWithInteger:(NSInteger)self],
		  class, address);
...
- (UIView *)findViewWithClass:(NSString *)class andX:(NSNumber *)x andY:(NSNumber *)y {
	NSLog(@"[Current view: %@ (addr: %@)] Looking for view: %@, with x: %@, and y:%@",
		  NSStringFromClass([self class]),
		  [NSNumber numberWithInteger:(NSInteger)self],
		  class, x, y);
	...
	if ([NSStringFromClass([self class]) isEqualToString:class]) {
		...
		if (((NSInteger) self.frame.origin.x) == [x intValue] && ((NSInteger) self.frame.origin.y) == [y intValue]) {
			NSLog(@"...x and y coordinates are equal too!");
			return self;
		}
	}
	
	for (UIView *subview in [self subviews]) {
		UIView* response = [subview findViewWithClass:class andX:x andY:y];
		if (response) {
			return response;
		}
	}
