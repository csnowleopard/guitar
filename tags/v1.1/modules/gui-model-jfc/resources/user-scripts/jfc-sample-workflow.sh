...
cmd="$SCRIPT_DIR/tc-gen-random.sh -e $efg_file -l $tc_length -m 200 -d $testcases_dir"
...
do
	...
	cmd="$SCRIPT_DIR/jfc-replayer.sh -cp $aut_classpath -c  $mainclass -g $gui_file -e $efg_file -t $testcase -i $intial_wait -d $relayer_delay -l $logs_dir/$test_name.log -gs $states_dir/$test_name.sta -cf $configuration -ts"
