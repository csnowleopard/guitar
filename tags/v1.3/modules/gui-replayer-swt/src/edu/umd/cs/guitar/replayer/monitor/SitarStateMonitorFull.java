 ...
 */
public class SitarStateMonitorFull extends StateMonitor {
	 ...
	 * {@inheritDoc}
	 * 
	 * This method is identical to {@code
	 * StateMonitorFull#afterStep(TestStepEndEventArgs)} except that it does not
	 * use Jemmy to wait until the event queue is empty, since SWTGuitar uses
	 * blocking actions on the GUI.
	 */
	@Override
	public void afterStep(GTestStepEventArgs aStep) {
		TestStepEndEventArgs stateStep = (TestStepEndEventArgs) aStep;

		GUITARLog.log.info("Recording GUI state....");

		List<StepType> lSteps = outTestCase.getStep();
		StepType step = stateStep.getStep();
		...
		if (windowsNew.size() > 0) {
			...
			List<PropertyType> ID = monitor
					.selectIDProperties(stateStep.component);
