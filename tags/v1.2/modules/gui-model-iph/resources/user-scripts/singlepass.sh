...
#sleep $xcode_build_time
#eval $run_iphonesim
kill_iph_skd="ps aux | grep i[pP]hone | grep -v 'perl' | awk '{print \$2}' | xargs -n 1 -I {} kill -9 {} &> /dev/null"
...
for testcase in `find $testcases_dir -name "*.tst"| head -n$testcase_num`  
    ...
    # Removed -c
   	...
   	cmd="$SCRIPT_DIR/iph-replayer.sh -cp $aut_classpath -g $gui_file -e $efg_file -t $aut_dir/Demo/testcases/$testcase -i $initial_wait -d $replayer_delay -so $replayer_so -l $logs_dir/$test_name.log -gs $states_dir/$test_name.sta -cf $SCRIPT_DIR/configuration.xml &> replayer.out &"
   	# adding application arguments if needed 
   	if [ ! -z $args ] 
   	then 
   		cmd="$cmd -a \"$args\" " 
